...
public class NotaDebitoRepository : GenericRepository<NotaDebito>, INotaDebitoRepository
{
    public NotaDebitoRepository(ApplicationDbContext context) : base(context) { }

    public async Task<NotaDebito?> GetWithMotivosAsync(long id) =>
    ...
    public async Task<(IReadOnlyList<NotaDebito> Items, int Total)> GetPagedAsync(int pagina, int cantidad, EstadoSRI? estado, long? facturaId)
    {
        var page = pagina <= 0 ? 1 : pagina;
        IQueryable<NotaDebito> query = _dbSet.AsNoTracking()
            .Include(n => n.Cliente)
